...
# lazy-load: seed read at call time
def _load_seed(): 
    p = BASE_DIR / "seed.txt"
    return p.read_text() if p.exists() else ""
# lazy-load: world read at call time
def _load_world():
    p = BASE_DIR / "world.md"
    return p.read_text() if p.exists() else ""
world = _load_world()
...
# Voice Part 2 only
# lazy-load: voice read at call time
def _load_voice():
    p = BASE_DIR / "voice.md"
    return p.read_text() if p.exists() else ""
voice = _load_voice()
...
def run(seed: str = "", world: str = "", voice_part2: str = ""):
    """Generate characters for a novel. Call this with actual seed/world/voice content."""
